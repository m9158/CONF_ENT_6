...
| 평가 지표 | 값 | 평가 |
|----------|-----|------|
| **MAE** | 125,414,630 KRW | 평균 1.25억 원 오차 |
| **RMSE** | 256,297,711 KRW | 평균 2.56억 원 오차 |
| **R² Score** | **0.7643** | ✅ **76.4% 설명력 (우수)** |
...
model_2/
│
├── README.md                    # 📘 본 문서 (프로젝트 개요 및 가이드)
│
├── ipynb/
│   └── main.ipynb              # 🔬 전체 구축 노트북 (7단계 파이프라인)
│                                #    - STEP 1~3: 데이터 준비 및 전처리
│                                #    - STEP 4: LSTM 모델 훈련
│                                #    - STEP 5~7: 시뮬레이션 및 저장
│
└── model/
    ├── model_Rb_best.h5        # 🏆 LSTM 모델 (Best Checkpoint, 권장)
    │                            #    - 용도: Rb(t) 예측 (자연 수익 곡선)
    │                            #    - 크기: ~100KB
    │                            #    - Validation Loss: 0.03731 (최저)
    │                            #    - 권장 용도: Part 3 시뮬레이션, 실전 배포
    │
    ├── model_2A_Rb_LSTM.h5     # 🧠 LSTM 모델 (Final Epoch)
    │                            #    - 용도: Rb(t) 예측 (최종 학습 상태)
    │                            #    - 권장 용도: Fine-tuning, 비교 분석
    │
    ├── scaler_X.pkl            # 📊 입력 피처 스케일러 (StandardScaler)
    │                            #    - 용도: 6개 입력 피처 정규화
    │                            #    - 필수: 예측 시 반드시 동일 스케일러 사용
    │
    ├── scaler_y.pkl            # 📊 타겟 변수 스케일러 (StandardScaler)
    │                            #    - 용도: daily_sales_amt 역정규화
    │                            #    - 필수: 예측 결과를 실제 금액으로 변환
    │
    ├── movie_meta_with_cannib.csv  # 📄 영화 메타데이터 (~500개)
    │                            #    - 용도: 기존 영화 정보 조회 (선택적)
    │                            #    - 컬럼: movieCd, TFS, ONS, gamma, cannib_coef
    │                            #    - 새로운 영화 시뮬레이션 시 불필요
    │
    └── model2_usage.md         # 📖 사용 가이드
                                 #    - 새로운 영화 시뮬레이션 방법
                                 #    - Part 3 통합 사용 방법
```
...
| 파일 | 용도 | 새 영화 시뮬레이션 | Part 3 통합 | 크기 |
|------|------|:------------------:|:-----------:|------|
| `model_Rb_best.h5` | Rb 예측 | ✅ **필수** | ✅ **필수** | ~100KB |
| `model_2A_Rb_LSTM.h5` | Rb 예측 (대안) | ⭕ 선택 | ⭕ 선택 | ~100KB |
| `scaler_X.pkl` | 입력 정규화 | ✅ **필수** | ✅ **필수** | ~5KB |
| `scaler_y.pkl` | 출력 역정규화 | ✅ **필수** | ✅ **필수** | ~1KB |
| `movie_meta_with_cannib.csv` | 기존 영화 참조 | ❌ 불필요 | ⭕ 편의용 | ~30KB |
| `model2_usage.md` | 사용 설명서 | ✅ 권장 | ✅ 권장 | ~20KB |
