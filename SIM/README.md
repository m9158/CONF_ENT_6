...
![Part 1 기본 분석](part1_individual_optimization.png)

**설명**: 샘플 영화(야마들)의 홀드백 기간(0~365일)에 따른 수익 변화 분석
- **상단 그래프**: 극장 수익(파랑), OTT 권리료(주황), 총 수익(녹색) 추이
- **하단 그래프**: 수익 구성 비율 (Stacked Area)
- **핵심 발견**: 최적 홀드백 161일, 극장 수익이 전체의 96% 차지

#### 2. 5개 샘플 영화 총수익 비교 (365일)

![Part 1 샘플 비교](part1_comparison_365days.png)

**설명**: 다양한 장르의 5개 영화(야마들, 정발산티마 연구소, 뉴 노멀, 대외비, 카운트)의 홀드백별 총수익 곡선
- 각 영화의 최적 홀드백 지점 표시 (★)
- **핵심 발견**: 영화별로 최적 홀드백이 161일~247일로 다양하게 분포

#### 3. 랜덤 샘플 20개 영화 상세 분석 (Dual Y-Axis)

![Part 1 Extended 샘플 20개](part1_extended_sample20.png)

**설명**: 무작위 추출된 20개 영화의 극장 수익과 OTT 권리료 곡선 비교
- **왼쪽 축(파랑)**: 극장 수익 (지수 감소 패턴)
- **오른쪽 축(주황)**: OTT 권리료 (홀드백 길수록 감소)
- **핵심 발견**: 두 수익원의 스케일 차이가 크므로 dual y-axis로 시각화 필요

#### 4. 수익 상위 10개 영화의 최적 홀드백

![Part 1 Extended Top 10](part1_extended_top10.png)

**설명**: 총수익 기준 상위 10개 영화의 최적 홀드백(상단)과 최대 수익(하단) 비교
- **상단**: 영화별 최적 홀드백 (145일~253일)
- **하단**: 최대 총수익 (837억~2,371억 원)
- **핵심 발견**: 블록버스터(외계+인 2부)는 최적 홀드백이 188일로 중간 수준

#### 5. TFS/ONS 매트릭스와 최적 홀드백 관계

![Part 1 Extended Matrix](part1_extended_matrix.png)

**설명**: 영화 특성(TFS, ONS)과 최적 홀드백(좌측) 및 총수익(우측)의 관계
- **좌측**: 색상으로 최적 홀드백 표현 (빨강=긴 홀드백, 파랑=짧은 홀드백)
- **우측**: 색상으로 총수익 표현 (노랑=높은 수익, 보라=낮은 수익)
- **핵심 발견**: TFS/ONS와 최적 홀드백 간의 명확한 상관관계는 제한적

#### 6. 전체 영화의 최적 홀드백 분포

![Part 1 Extended Distribution](part1_extended_distribution.png)

**설명**: 전체 영화 데이터의 최적 홀드백 분포와 TFS/ONS 관계 분석 (4개 서브플롯)
- **좌상**: 최적 홀드백 히스토그램 (평균 232일, 중앙값 247일)
- **우상**: TFS vs 최적 홀드백 (ONS로 색상 구분)
- **좌하**: ONS vs 최적 홀드백 (TFS로 색상 구분)
- **우하**: 홀드백 범위별 총수익 박스플롯
- **핵심 발견**: 대부분 영화가 180일 이상의 긴 홀드백으로 수렴 → 미시적 최적화의 한계

---

### Part 2: 영화 유형별 충돌 분석

#### 7. TFS/ONS 매트릭스와 유형별 최적 홀드백

![Part 2 기본 분석](part2_tfs_ons_matrix.png)

**설명**: 영화 유형(Theater-Exclusive, Hybrid, OTT-Native, Niche)별 최적 홀드백 분석
- **좌측**: TFS/ONS 산점도 (색상은 최적 홀드백)
- **우측**: 유형별 최적 홀드백 박스플롯
- **핵심 발견**: Theater-Exclusive 영화는 평균 171일, 모든 유형이 장기 홀드백으로 수렴

#### 8. Part 2 Extended: 미시적 최적화의 충돌 구조

![Part 2 Extended 충돌](part2_extended_conflict.png)

**설명**: 영화 유형별 대표작에서 30일(빠른출시) vs 180일(긴보호) 시나리오의 4대 이해관계자 효용 비교
- **Theater-Exclusive**: 배급사는 180일 선호, 소비자는 불만
- **Hybrid**: 배급사 최적(180일) 시 OTT 효용이 95% 감소 (257억 → 13억)
- **Niche**: 독립 제작사가 180일에서 시간 할인 압박
- **핵심 발견**: 배급사 미시적 최적 ≠ 산업 거시적 최적 (수치로 입증)

---

### Part 3: 정책 시뮬레이션

#### 9. 정책별 이해관계자 효용 및 산업 전체 효용 비교

![Part 3 정책 비교](part3_policy_comparison.png)

**설명**: 3개 정책 시나리오(Laissez-faire, Uniform_180, Dynamic)의 효용 비교
- **좌측**: 정책별 이해관계자 효용 (Stacked Bar)
  - 배급사/극장 (파랑)
  - 독립 제작사 (주황)
  - 국내 OTT (녹색)
  - 소비자 (노랑)
- **우측**: 산업 전체 효용 비교 (Bar Chart)
- **핵심 발견**: 
  - **Laissez-faire (자율)**: 4.14조 원, 배급사 효용 최대
  - **Uniform_180 (일괄 규제)**: 3.78조 원, 소비자 불만 높음
  - **Dynamic (동적 차등)**: 4.48조 원 🏆, 산업 전체 효용 최대
